...
from fast_exact_topk_core.bitonic.topk import (
  bitonic_topk_arrays as _bitonic_topk_arrays,
)
from fast_exact_topk_core.bitonic import bitonic_topk_in_vmem as bitonic_topk

from fast_exact_topk_core.divide_and_filter_topk.convergence_theory import (
...
def _tpu_repeat(x, repeats, axis):
  repeat = getattr(pltpu, "repeat", None)
  if repeat is not None:
    return repeat(x, repeats, axis=axis)
  reps = [1] * x.ndim
  reps[axis] = repeats
...
def _merge_unconverged_bins_topk(
  ...
  *,
  ...
  ) * num_bins + _tpu_repeat(packing_perm, n // NUM_LANES, axis=1)
